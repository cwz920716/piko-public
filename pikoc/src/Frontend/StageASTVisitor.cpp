...
bool StageASTVisitor::VisitCXXRecordDecl(clang::CXXRecordDecl *decl) {
  ...
  clang::CXXBaseSpecifier* base = nullptr;
...
bool StageASTVisitor::addAssignBinSummary(clang::CXXMethodDecl *m, stageSummary *ssum) {
  ...
  if(funcBody->children().begin() == funcBody->children().end()) {
    policyFound = true;
    assignSum.policy = assignEmpty;
    assignSum.trivial = true;
  }

  for(clang::Stmt::child_iterator range = funcBody->child_begin(),
      range_end = funcBody->child_end(); range != range_end; ++range) {
    numChildren += 1;

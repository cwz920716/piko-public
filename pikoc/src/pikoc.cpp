                                                         ...
                                                         bool optimize)
{
  ...
  for(int i = 0; i < psum.stagesInOrder.size(); ++i) {
    ...
    // if cannnot fuse with previous stage, starts a new kernel, a kenrel is
    // a LIST of stages
...
void generateKernels(PipeSummary psum, std::ostream &outfile,
  ...
  for(std::vector< std::vector<stageSummary*> >::iterator
    ...
    // TODO(wcui) why loop will require atomics?
    // because fetchPrimAtomic actually decrenment numPrims and only looped
    // stage need that because they will reuse bins
    // imagine a self loop
    if(psum.hasLoop)
    ...
    // TODO(wcui) why unlooped stage need piko::BinSynchronize() and looped
    // stage does not need it?
    if(!psum.hasLoop) {
